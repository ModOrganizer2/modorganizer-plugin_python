 ...
 * object, while __getattr__ will delegate to the underlying QClass object when
 * required.
 *
 * This allow access to Qt interface for object exposed using boost::python (e.g.,
 * signals, methods from QObject or QWidget, etc.).
 */
#define Q_DELEGATE(Class, QClass, Name)                                                \
    .def(                                                                              \
        ...
        .def(                                                                          \
            "__getattr__", +[](Class* w, bpy::str str) -> bpy::object {                \
...
BOOST_PYTHON_MODULE(mobase)
{
    ...
    bpy::implicitly_convertible<std::shared_ptr<FileTreeEntry>,
    ...
    bpy::implicitly_convertible<std::shared_ptr<IFileTree>,
    ...
    utils::register_sequence_container<
        QList<QVariant>>();  // Required for QVariant since this is QVariantList.
    utils::register_sequence_container<
    ...
    utils::register_associative_container<
        QMap<QString, QVariant>>();  // Required for QVariant since this is QVariantMap.
    ...
    bpy::register_tuple<std::tuple<IPluginInstaller::EInstallResult,
                                   std::shared_ptr<IFileTree>, QString, int>>();
    bpy::register_tuple<std::tuple<QString, QString>>();

    // Variants:
    bpy::register_variant<
    ...
    // Functions:
    utils::register_functor_converter<void()>();  // converter for the
    ...
    utils::register_functor_converter<void(
        const QString&, int,
        int)>();  // converter for the onModMoved-callback and onPluginMoved callbacks
    utils::register_functor_converter<void(
        const std::map<QString, IModList::ModStates>&)>();  // converter for the
                                                            // onModStateChanged-callback
                                                            // (IModList)
    utils::register_functor_converter<void(
        const std::map<QString, IPluginList::PluginStates>&)>();  // converter for the
                                                                  // onPluginStateChanged-callback
                                                                  // (IPluginList)
    utils::register_functor_converter<void(const QString&, const QString&,
                                           const QVariant&, const QVariant&)>();
    utils::register_functor_converter<void(QMainWindow*)>();
    utils::register_functor_converter<void(IProfile*),
                                      bpy::pointer_wrapper<IProfile*>>();
    utils::register_functor_converter<void(IProfile*, const QString&, const QString&),
                                      bpy::pointer_wrapper<IProfile*>>();
    utils::register_functor_converter<void(IProfile*, IProfile*),
                                      bpy::pointer_wrapper<IProfile*>>();
    utils::register_functor_converter<bool(const QString&)>();
    utils::register_functor_converter<IFileTree::WalkReturn(
        const QString&, std::shared_ptr<const FileTreeEntry>)>();
    utils::register_functor_converter<
        bool(const IOrganizer::FileInfo&),
        boost::reference_wrapper<const IOrganizer::FileInfo&>>();
    ...
    utils::register_functor_converter<void(IModInterface*),
                                      bpy::pointer_wrapper<IModInterface*>>();
    utils::register_functor_converter<bool(IOrganizer*),
                                      bpy::pointer_wrapper<IOrganizer*>>();
    utils::register_functor_converter<void(const IPlugin*),
                                      bpy::pointer_wrapper<const IPlugin*>>();

    // This one is kept for backward-compatibility while we deprecate onModStateChanged
    // for singl mod.
    utils::register_functor_converter<void(
        const QString&,
        IModList::ModStates)>();  // converter for the onModStateChanged-callback
                                  // (IModList).
    utils::register_functor_converter<void(
        const QString&,
        IPluginList::PluginStates)>();  // converter for the
                                        // onPluginStateChanged-callback (IPluginList).
    ...
    bpy::class_<VersionInfo>("VersionInfo")
        ...
        // Note: Order of the two init<> below is important because ReleaseType is a
        // simple enum with an implicit int conversion.
        .def(bpy::init<int, int, int, int, VersionInfo::ReleaseType>(
            (bpy::arg("major"), "minor", "subminor", "subsubminor",
             bpy::arg("release_type") = VersionInfo::RELEASE_FINAL)))
        .def(bpy::init<int, int, int, VersionInfo::ReleaseType>(
            (bpy::arg("major"), "minor", "subminor",
             bpy::arg("release_type") = VersionInfo::RELEASE_FINAL)))
        .def("clear", &VersionInfo::clear)
        .def("parse", &VersionInfo::parse,
             (bpy::arg("value"), bpy::arg("scheme") = VersionInfo::SCHEME_DISCOVER,
              bpy::arg("is_manual") = false))
        .def("canonicalString", &VersionInfo::canonicalString)
        .def("displayString", &VersionInfo::displayString,
             bpy::arg("forced_segments") = 2)
    ...
    bpy::class_<ExecutableInfo>(
        ...
        .def("withWorkingDirectory", &ExecutableInfo::withWorkingDirectory,
             bpy::return_self<>(), bpy::arg("directory"))
    ...
    bpy::class_<ExecutableForcedLoadSetting>(
        ...
        .def("withForced", &ExecutableForcedLoadSetting::withForced,
             bpy::return_self<>(), bpy::arg("forced"))
        .def("withEnabled", &ExecutableForcedLoadSetting::withEnabled,
             bpy::return_self<>(), bpy::arg("enabled"))
    ...
    bpy::class_<ISaveGameWrapper, bpy::bases<>, boost::noncopyable>("ISaveGame")
        ...
        .def("getSaveGroupIdentifier",
             bpy::pure_virtual(&ISaveGame::getSaveGroupIdentifier))
        .def("allFiles", bpy::pure_virtual(&ISaveGame::allFiles));

    // See Q_DELEGATE for more details.
    bpy::class_<ISaveGameInfoWidgetWrapper, bpy::bases<>, ISaveGameInfoWidgetWrapper*,
                boost::noncopyable>(
    ...
    // Plugin requirements:
    auto iPluginRequirementClass =
        bpy::class_<IPluginRequirementWrapper, bpy::bases<>, boost::noncopyable>(
            "IPluginRequirement");
    {
        bpy::scope scope = iPluginRequirementClass;

        bpy::class_<IPluginRequirement::Problem>(
            "Problem", bpy::init<QString, QString>((bpy::arg("short_description"),
                                                    bpy::arg("long_description") = "")))
        ...
        iPluginRequirementClass.def("check",
                                    bpy::pure_virtual(&IPluginRequirement::check),
                                    bpy::arg("organizer"));
    }

    bpy::class_<PluginRequirementFactory, boost::noncopyable>(
        ...
        .def("basic", &PluginRequirementFactory::basic,
             (bpy::arg("checker"), "description"))
        .staticmethod("basic");

    bpy::class_<IOrganizer::FileInfo>("FileInfo", bpy::init<>())
        .add_property(
            "filePath",
            +[](const IOrganizer::FileInfo& info) {
                return info.filePath;
            },
            +[](IOrganizer::FileInfo& info, QString value) {
                info.filePath = value;
            })
        .add_property(
            "archive",
        ...
        .add_property(
            ...
            +[](IOrganizer::FileInfo& info, QStringList value) {
                info.origins = value;
            });
    ...
    bpy::class_<IOrganizer, boost::noncopyable>("IOrganizer", bpy::no_init)
        ...
        .def("createMod", &IOrganizer::createMod,
             bpy::return_value_policy<bpy::reference_existing_object>(),
             bpy::arg("name"))
        .def("getGame", &IOrganizer::getGame,
             bpy::return_value_policy<bpy::reference_existing_object>(),
             bpy::arg("name"))
        .def("modDataChanged", &IOrganizer::modDataChanged, bpy::arg("mod"))
        .def(
            "isPluginEnabled",
            +[](IOrganizer* o, IPlugin* plugin) {
                return o->isPluginEnabled(plugin);
            },
            bpy::arg("plugin"))
        .def(
            "isPluginEnabled",
            +[](IOrganizer* o, QString const& plugin) {
                return o->isPluginEnabled(plugin);
            },
            bpy::arg("plugin"))
        .def("pluginSetting", &IOrganizer::pluginSetting,
             (bpy::arg("plugin_name"), "key"))
        .def("setPluginSetting", &IOrganizer::setPluginSetting,
             (bpy::arg("plugin_name"), "key", "value"))
        .def("persistent", &IOrganizer::persistent,
             (bpy::arg("plugin_name"), "key", bpy::arg("default") = QVariant()))
        .def("setPersistent", &IOrganizer::setPersistent,
             (bpy::arg("plugin_name"), "key", "value", bpy::arg("sync") = true))
        .def("pluginDataPath", &IOrganizer::pluginDataPath)
        .def("installMod", &IOrganizer::installMod,
             bpy::return_value_policy<bpy::reference_existing_object>(),
             (bpy::arg("filename"), bpy::arg("name_suggestion") = ""))
        ...
        .def(
            ...
            +[](const IOrganizer* o, QString const& p,
                std::function<bool(QString const&)> f) {
                return o->findFiles(p, f);
            },
            (bpy::arg("path"), "filter"))

        // In C++, it is possible to create a QStringList implicitly from a single
        // QString. This is not possible with the current converters in python (and I do
        // not think it is a good idea to have it everywhere), but here it is nice to be
        // able to pass a single string, so we add an extra overload. Important: the
        // order matters, because a Python string can be converted to a QStringList
        // since it is a sequence of single-character strings:
        .def(
            "findFiles",
        ...
        // Custom implementation for startApplication and waitForApplication because 1)
        // HANDLE (= void*) is not properly converted from/to python, and 2) we need to
        // convert the by-ptr argument to a return-tuple for waitForApplication:
        .def(
            "startApplication",
            ...
            (bpy::arg("executable"), (bpy::arg("args") = QStringList()),
             (bpy::arg("cwd") = ""), (bpy::arg("profile") = ""),
             (bpy::arg("forcedCustomOverwrite") = ""),
             (bpy::arg("ignoreCustomOverwrite") = false)),
        ...
        .def(
            "onPluginEnabled",
            +[](IOrganizer* o, QString const& name, std::function<void()> const& func) {
        ...
        .def(
            ...
            +[](IOrganizer* o, QString const& name) {
                utils::show_deprecation_warning(
        ...
        .def(
            ...
            +[](IOrganizer* o, IModInterface* mod) {
                utils::show_deprecation_warning(
                    "removeMod", "IOrganizer::removeMod(IModInterface) is deprecated, "
                                 "use IModList::removeMod(IModInterface) instead.");
        ...
        .def(
            ...
            +[](IOrganizer* o, bool s) {
                utils::show_deprecation_warning(
                    "refreshModList", "IOrganizer::refreshModList(bool) is deprecated, "
                                      "use IOrganizer::refresh(bool) instead.");
                o->refresh(s);
            },
    ...
    auto fileTreeEntryClass =
        bpy::class_<FileTreeEntry, boost::noncopyable>("FileTreeEntry", bpy::no_init);
    {
        ...
        fileTreeEntryClass
            ...
            // Forcing the conversion to FileTypeS to avoid having to expose FileType in
            // python:
            .def(
                "fileType",
                +[](FileTreeEntry* p) {
                    return FileTreeEntry::FileTypes{p->fileType()};
                })
            // This should probably not be exposed in python since we provide automatic
            // downcast: .def("getTree",
            ...
            .def(
                ...
                bpy::arg("suffix"))
            .def("parent",
                 static_cast<std::shared_ptr<IFileTree> (FileTreeEntry::*)()>(
                     &FileTreeEntry::parent),
                 "[optional]")
            .def("path", &FileTreeEntry::path, bpy::arg("sep") = "\\")
            .def("pathFrom", &FileTreeEntry::pathFrom,
                 (bpy::arg("tree"), bpy::arg("sep") = "\\"))
            ...
            .def(
                "__repr__", +[](const FileTreeEntry* entry) {
    ...
    // IFileTree scope:
    auto iFileTreeClass =
        bpy::class_<IFileTree, bpy::bases<FileTreeEntry>, boost::noncopyable>(
            "IFileTree", bpy::no_init);
    {
        ...
        iFileTreeClass
            ...
            .def("exists",
                 static_cast<bool (IFileTree::*)(QString, IFileTree::FileTypes) const>(
                     &IFileTree::exists),
                 (bpy::arg("path"), bpy::arg("type") = IFileTree::FILE_OR_DIRECTORY))
            .def("find",
                 static_cast<std::shared_ptr<FileTreeEntry> (IFileTree::*)(
                     QString, IFileTree::FileTypes)>(&IFileTree::find),
                 bpy::return_value_policy<
                     utils::downcast_return<FileTreeEntry, IFileTree>>(),
                 (bpy::arg("path"), bpy::arg("type") = IFileTree::FILE_OR_DIRECTORY),
                 "[optional]")
            .def("pathTo", &IFileTree::pathTo,
                 (bpy::arg("entry"), bpy::arg("sep") = "\\"))

            // Note: walk() would probably be better as a generator in python, but it is
            // likely impossible to construct from the C++ walk() method.
            .def("walk", &IFileTree::walk,
                 (bpy::arg("callback"), bpy::arg("sep") = "\\"))
            ...
            // Merge needs custom return types depending if the user wants overrides or
            // not. A failure is translated into an exception for easier tracing and
            // handling.
            .def(
                "merge",
            ...
            // Insert and erase returns an iterator, which makes no sense in python, so
            // we convert it to bool. Erase is also renamed "remove" since "erase" is
            // very C++.
            .def(
                "insert",
            ...
            .def(
                ...
                (bpy::arg("entry"), bpy::arg("path") = "",
                 bpy::arg("insert_policy") = IFileTree::InsertPolicy::FAIL_IF_EXISTS))
            ...
            .def(
                "__getitem__",
                static_cast<std::shared_ptr<FileTreeEntry> (IFileTree::*)(std::size_t)>(
                    &IFileTree::at),
                bpy::return_value_policy<
                    utils::downcast_return<FileTreeEntry, IFileTree>>())
            .def("__iter__", bpy::range<bpy::return_value_policy<
                                 utils::downcast_return<FileTreeEntry, IFileTree>>>(
                                 static_cast<IFileTree::iterator (IFileTree::*)()>(
                                     &IFileTree::begin),
                                 static_cast<IFileTree::iterator (IFileTree::*)()>(
                                     &IFileTree::end)))
    ...
    bpy::class_<IModRepositoryBridge, boost::noncopyable>("IModRepositoryBridge",
        ...
        .def("requestToggleEndorsement",
             ...
             (bpy::arg("game_name"), "mod_id", "mod_version", "endorse", "user_data"))
    ...
    bpy::class_<ModRepositoryFileInfo>("ModRepositoryFileInfo", bpy::no_init)
        ...
        .def(bpy::init<bpy::optional<QString, int, int>>(
            (bpy::arg("game_name"), "mod_id", "file_id")))
        .def("__str__", &ModRepositoryFileInfo::toString)
        .def("createFromJson", &ModRepositoryFileInfo::createFromJson, bpy::arg("data"))
    ...
    bpy::class_<IDownloadManager, boost::noncopyable>("IDownloadManager", bpy::no_init)
        ...
        .def("onDownloadPaused", &IDownloadManager::onDownloadPaused,
             bpy::arg("callback"))
        .def("onDownloadFailed", &IDownloadManager::onDownloadFailed,
             bpy::arg("callback"))
        .def("onDownloadRemoved", &IDownloadManager::onDownloadRemoved,
             bpy::arg("callback"));

    bpy::class_<IInstallationManager, boost::noncopyable>("IInstallationManager",
                                                          bpy::no_init)
        .def("getSupportedExtensions", &IInstallationManager::getSupportedExtensions)
        .def("extractFile", &IInstallationManager::extractFile,
             (bpy::arg("entry"), bpy::arg("silent") = false))
        .def("extractFiles", &IInstallationManager::extractFiles,
             (bpy::arg("entries"), bpy::arg("silent") = false))
        .def(
            "createFile",
            +[](IInstallationManager* m,
                std::shared_ptr<const MOBase::FileTreeEntry> entry) {
                return m->createFile(utils::clean_shared_ptr(entry));
            },
            bpy::arg("entry"))

        // accept both QString and GuessedValue<QString> since the conversion is not
        // automatic in Python, and return a tuple to get back the mod name and the mod
        // ID
        .def(
            "installArchive",
            +[](IInstallationManager* m,
                std::variant<QString, GuessedValue<QString>> modName, QString archive,
                int modId) {
    ...
    bpy::class_<MOBase::GuessedValue<QString>, boost::noncopyable>("GuessedString")
        ...
        .def(bpy::init<QString const&, EGuessQuality>(
            (bpy::arg("value"), bpy::arg("quality") = EGuessQuality::GUESS_USER)))
        .def("update",
        ...
        .def("update",
             static_cast<GuessedValue<QString>& (
                 GuessedValue<QString>::*)(const QString&, EGuessQuality)>(
                 &GuessedValue<QString>::update),
        ...
        .def(
            ...
            bpy::return_self<>(), bpy::arg("other"))

        // Use an intermediate lambda to avoid having to register the std::function
        // conversion:
        .def(
            "setFilter",
            +[](GuessedValue<QString>* gv,
    ...
    bpy::class_<IPluginList, boost::noncopyable>("IPluginList", bpy::no_init)
        ...
        .def("setPriority", &MOBase::IPluginList::setPriority,
             (bpy::arg("name"), "priority"))
        .def("loadOrder", &MOBase::IPluginList::loadOrder, bpy::arg("name"))
        .def("hasMasterExtension", &MOBase::IPluginList::hasMasterExtension,
             bpy::arg("name"))
        .def("hasLightExtension", &MOBase::IPluginList::hasLightExtension,
             bpy::arg("name"))
        ...
        .def(
            ...
            +[](IPluginList* modList,
                ...
                utils::show_deprecation_warning(
                    "onPluginStateChanged",
                    "onPluginStateChanged(Callable[[str, IPluginList.PluginStates], "
                    "None]) is deprecated, "
    ...
    bpy::class_<IModList, boost::noncopyable>("IModList", bpy::no_init)
        ...
        .def("allModsByProfilePriority", &MOBase::IModList::allModsByProfilePriority,
             bpy::arg("profile") = bpy::ptr((IProfile*)nullptr))
        ...
        .def("renameMod", &MOBase::IModList::renameMod,
             bpy::return_value_policy<bpy::reference_existing_object>(),
             (bpy::arg("mod"), bpy::arg("name")))

        .def("state", &MOBase::IModList::state, bpy::arg("name"))
        .def("setActive",
             static_cast<int (IModList::*)(QStringList const&, bool)>(
                 &MOBase::IModList::setActive),
             (bpy::arg("names"), "active"))
        .def("setActive",
             static_cast<bool (IModList::*)(QString const&, bool)>(
                 &MOBase::IModList::setActive),
             (bpy::arg("name"), "active"))
        ...
        .def(
            ...
            +[](IModList* modList,
                ...
                utils::show_deprecation_warning(
                    "onModStateChanged", "onModStateChanged(Callable[[str, "
                                         "IModList.ModStates], None]) is deprecated, "
    ...
    // Note: localizedName, master, requirements and enabledByDefault have to go in all
    // the plugin wrappers declaration, since the default functions are specific to each
    // wrapper, otherwise in turns into an infinite recursion mess.
    bpy::class_<IPluginWrapper, boost::noncopyable>("IPlugin")
        ...
        .def("localizedName", &MOBase::IPlugin::localizedName,
             &IPluginWrapper::localizedName_Default)
        ...
        .def("requirements", &MOBase::IPlugin::requirements,
             &IPluginWrapper::requirements_Default)
    ...
    bpy::class_<IPluginDiagnoseWrapper, bpy::bases<IPlugin>, boost::noncopyable>(
        "IPluginDiagnose")
        .def("localizedName", &MOBase::IPlugin::localizedName,
             &IPluginDiagnoseWrapper::localizedName_Default)
        .def("master", &MOBase::IPlugin::master,
             &IPluginDiagnoseWrapper::master_Default)
        .def("requirements", &MOBase::IPlugin::requirements,
        ...
        .def("activeProblems",
             bpy::pure_virtual(&MOBase::IPluginDiagnose::activeProblems))
        .def("shortDescription",
             bpy::pure_virtual(&MOBase::IPluginDiagnose::shortDescription),
             bpy::arg("key"))
        .def("fullDescription",
             bpy::pure_virtual(&MOBase::IPluginDiagnose::fullDescription),
             bpy::arg("key"))
        .def("hasGuidedFix", bpy::pure_virtual(&MOBase::IPluginDiagnose::hasGuidedFix),
             bpy::arg("key"))
        .def("startGuidedFix",
             bpy::pure_virtual(&MOBase::IPluginDiagnose::startGuidedFix),
             bpy::arg("key"))
    ...
    bpy::class_<Mapping>("Mapping", bpy::init<>())
        .def("__init__",
             bpy::make_constructor(
                 +[](QString src, QString dst, bool dir, bool crt) -> Mapping* {
                     return new Mapping{src, dst, dir, crt};
                 },
                 bpy::default_call_policies(),
                 (bpy::arg("source"), bpy::arg("destination"), bpy::arg("is_directory"),
                  bpy::arg("create_target") = false)))
    ...
    bpy::class_<IPluginFileMapperWrapper, bpy::bases<IPlugin>, boost::noncopyable>(
        "IPluginFileMapper")
        .def("localizedName", &MOBase::IPlugin::localizedName,
             &IPluginFileMapperWrapper::localizedName_Default)
        .def("master", &MOBase::IPlugin::master,
             &IPluginFileMapperWrapper::master_Default)
        .def("requirements", &MOBase::IPlugin::requirements,
    ...
    // This doesn't actually do the conversion, but might be convenient for accessing
    // the names for enum bits
    ...
    bpy::class_<IPluginGameWrapper, bpy::bases<IPlugin>, boost::noncopyable>(
        "IPluginGame")
        .def("localizedName", &MOBase::IPlugin::localizedName,
             &IPluginGameWrapper::localizedName_Default)
        ...
        .def("initializeProfile",
             bpy::pure_virtual(&MOBase::IPluginGame::initializeProfile),
             (bpy::arg("directory"), "settings"))
        .def("listSaves", bpy::pure_virtual(&MOBase::IPluginGame::listSaves),
             bpy::arg("folder"))
        ...
        .def("documentsDirectory",
             bpy::pure_virtual(&MOBase::IPluginGame::documentsDirectory))
        ...
        .def("executableForcedLoads",
             bpy::pure_virtual(&MOBase::IPluginGame::executableForcedLoads))
        ...
        .def("validShortNames",
             bpy::pure_virtual(&MOBase::IPluginGame::validShortNames))
        ...
        .def("loadOrderMechanism",
             bpy::pure_virtual(&MOBase::IPluginGame::loadOrderMechanism))
        .def("sortMechanism", bpy::pure_virtual(&MOBase::IPluginGame::sortMechanism))
        .def("nexusModOrganizerID",
             bpy::pure_virtual(&MOBase::IPluginGame::nexusModOrganizerID))
        ...
        .def("getLauncherName",
             bpy::pure_virtual(&MOBase::IPluginGame::getLauncherName))
            ...
            +[](MOBase::IPluginGame* p) {
                ...
                mp11::mp_for_each<
                    ...
                    [&](auto* pt) {
                        ...
                        typename bpy::reference_existing_object::apply<T*>::type
                            converter;

                        // Retrieve the python class object:
                        const bpy::converter::registration* registration =
                            bpy::converter::registry::query(bpy::type_id<T>());
                        bpy::object key = bpy::object(bpy::handle<>(
                            bpy::borrowed(registration->get_class_object())));
        ...
        .def(
            "feature",
            +[](MOBase::IPluginGame* p, bpy::object clsObj) {
                bpy::object feature;
                mp11::mp_for_each<
                    // Must user pointers because mp_for_each construct object:
                    mp11::mp_transform<std::add_pointer_t, MpGameFeaturesList>>(
                    [&](auto* pt) {
                        ...
                        if (clsObj.ptr() ==
                            (PyObject*)registration->get_class_object()) {
                            feature =
                                bpy::object(bpy::handle<>(converter(p->feature<T>())));
                        }
                    });
                return feature;
            },
    ...
    bpy::class_<IPluginInstaller, bpy::bases<IPlugin>, boost::noncopyable>(
        "IPluginInstaller", bpy::no_init)
        ...
        .def("onInstallationStart", &IPluginInstaller::onInstallationStart,
             (bpy::arg("archive"), bpy::arg("reinstallation"), bpy::arg("current_mod")))
        .def("onInstallationEnd", &IPluginInstaller::onInstallationEnd,
             (bpy::arg("result"), bpy::arg("new_mod")))
    ...
    bpy::class_<IPluginInstallerSimpleWrapper, bpy::bases<IPluginInstaller>,
        ...
        // Note: Keeping the variant here even if we always return a tuple to be
        // consistent with the wrapper and have proper stubs generation.
        .def(
            "install",
        ...
        .def("_parentWidget", &IPluginInstallerSimpleWrapper::parentWidget,
             bpy::return_value_policy<bpy::return_by_value>())
        .def("_manager", &IPluginInstallerSimpleWrapper::manager,
             bpy::return_value_policy<bpy::reference_existing_object>());

    bpy::class_<IPluginInstallerCustomWrapper, bpy::bases<IPluginInstaller>,
        ...
        .def("isArchiveSupported", &IPluginInstallerCustom::isArchiveSupported,
             bpy::arg("archive_name"))
        .def("supportedExtensions", &IPluginInstallerCustom::supportedExtensions)
        .def("install", &IPluginInstallerCustom::install,
             (bpy::arg("mod_name"), "game_name", "archive_name", "version", "nexus_id"))
        .def("_parentWidget", &IPluginInstallerSimpleWrapper::parentWidget,
             bpy::return_value_policy<bpy::return_by_value>())
        .def("_manager", &IPluginInstallerCustomWrapper::manager,
             bpy::return_value_policy<bpy::reference_existing_object>());

    bpy::class_<IPluginModPageWrapper, bpy::bases<IPlugin>, boost::noncopyable>(
        "IPluginModPage")
        .def("localizedName", &MOBase::IPlugin::localizedName,
             &IPluginModPageWrapper::localizedName_Default)
        ...
        .def("useIntegratedBrowser",
             bpy::pure_virtual(&IPluginModPage::useIntegratedBrowser))
        .def("handlesDownload", bpy::pure_virtual(&IPluginModPage::handlesDownload),
             (bpy::arg("page_url"), "download_url", "fileinfo"))
        .def("setParentWidget", &IPluginModPage::setParentWidget,
             &IPluginModPageWrapper::setParentWidget_Default, bpy::arg("parent"))
        .def("_parentWidget", &IPluginModPageWrapper::parentWidget,
             bpy::return_value_policy<bpy::return_by_value>());

    bpy::class_<IPluginPreviewWrapper, bpy::bases<IPlugin>, boost::noncopyable>(
        "IPluginPreview")
        .def("localizedName", &MOBase::IPlugin::localizedName,
             &IPluginPreviewWrapper::localizedName_Default)
        ...
        .def("supportedExtensions",
             bpy::pure_virtual(&IPluginPreview::supportedExtensions))
        .def("genFilePreview", bpy::pure_virtual(&IPluginPreview::genFilePreview),
             bpy::return_value_policy<bpy::return_by_value>(),
             (bpy::arg("filename"), "max_size"));

    bpy::class_<IPluginToolWrapper, bpy::bases<IPlugin>, boost::noncopyable>(
        "IPluginTool")
        .def("localizedName", &MOBase::IPlugin::localizedName,
             &IPluginToolWrapper::localizedName_Default)
        .def("master", &MOBase::IPlugin::master, &IPluginToolWrapper::master_Default)
        .def("requirements", &MOBase::IPlugin::requirements,
             &IPluginToolWrapper::requirements_Default)
        .def("enabledByDefault", &MOBase::IPlugin::enabledByDefault,
        ...
        .def("setParentWidget", &IPluginTool::setParentWidget,
             &IPluginToolWrapper::setParentWidget_Default, bpy::arg("parent"))
    ...
    // Expose MoVariant: MoVariant is a fake object whose only purpose is to be used as
    // a type-hint on the python side (e.g., def foo(x: mobase.MoVariant)). The real
    // MoVariant is defined in the generated stubs, since it's only relevant when doing
    // type-checking, but this needs to be defined, otherwise MoVariant is not found
    // when actually running plugins through MO2, making them crash.
...
struct PrintWrapper {
    void write(const char* message)
    {
        ...
        if (buffer.tellp() != 0 && buffer.str().back() == '\n') {
            // actually put the string in a variable so it doesn't get destroyed as soon
            // as we get a pointer to its data
...
BOOST_PYTHON_MODULE(moprivate)
{
    ...
    bpy::class_<ErrWrapper, boost::noncopyable>("ErrWrapper", bpy::init<>())
        .def("instance", &ErrWrapper::instance,
             bpy::return_value_policy<bpy::reference_existing_object>())
        .staticmethod("instance")
        .def("write", &ErrWrapper::write)
        .def("startRecordingExceptionMessage",
             &ErrWrapper::startRecordingExceptionMessage)
        .def("stopRecordingExceptionMessage",
    ...
    // Expose a function to create a particular tree, only for debugging purpose, not in
    // mobase.
    bpy::def(
        "makeTree",
        +[](std::function<bool(QString, bool)> callback) -> std::shared_ptr<IFileTree> {
            struct FileTree : IFileTree {

                using callback_t = std::function<bool(QString, bool)>;

                FileTree(std::shared_ptr<const IFileTree> parent, QString name,
                         callback_t callback)
            ...
            protected:
                ...
                {
                    return std::make_shared<FileTree>(nullptr, name(), m_Callback);
                }
...
bool handled_exec_file(bpy::str filename, bpy::object globals = bpy::object(),
                       bpy::object locals = bpy::object())
{
    return bpy::handle_exception(
        std::bind<bpy::object (&)(bpy::str, bpy::object, bpy::object)>(
            bpy::exec_file, filename, globals, locals));
}
...
QList<QObject*> PythonRunner::load(const QString& identifier)
{
    ...
    // `pluginName` can either be a python file (single-file plugin or a folder (whole
    // module).
    //
    // For whole module, we simply add the parent folder to path, then we load the
    // module with a simple bpy::import, and we retrieve the associated __dict__ from
    // which we extract either createPlugin or createPlugins.
    //
    // For single file, we need to use bpy::exec_file, and we will use the context
    // (global variables) from __main__ (already contains mobase, and other required
    // module). Since the context is shared between called of `instantiate`, we need to
    // make sure to remove createPlugin(s) from previous call.
    try {
        ...
        if (identifier.endsWith(".py")) {
            bpy::object mainModule = bpy::import("__main__");
            bpy::dict moduleNamespace =
                bpy::extract<bpy::dict>(mainModule.attr("__dict__"))();
        ...
        for (bpy::object pluginObj : plugins) {
            ...
            // Must try the wrapper because it's only a plugin extension interface in
            // C++, so doesn't extend QObject
...
void PythonRunner::unload(const QString& identifier)
{
    ...
    if (it != m_PythonObjects.end()) {
                ...
                if (PyObject_HasAttrString(mod.ptr(), "__path__")) {
                    ...
                    if (!folder.relativeFilePath(mpath).startsWith("..")) {
                        ...
                        log::debug("Unloading module {} from {} for {}.",
                                   bpy::extract<QString>(keys[i])(), mpath, identifier);
        ...
        // Boost.Python does not handle cyclic garbace collection, so we need to release
        // everything hold by the objects before deleting the objects themselves (done
        // when erasing from m_PythonObjects).
