...
#include "wrappers/uibase_wrappers.h"
 ...
 * methods: __getattr__ and Name, where Name will simply return the object as a
 * QClass* object, while __getattr__ will delegate to the underlying QClass
...
auto& add_qt_delegate(py::class_<Class, Args...>& pyclass, const char* name)
{
    return pyclass
        .def(name,
             [](Class* w) -> QClass* {
        ...
        .def(
            "__getattr__", +[](Class* w, py::str str) -> py::object {
                return py::cast((QClass*)w).attr(str);
            });
}
...
PYBIND11_MODULE(mobase, m)
{
    ...
    py::enum_<MOBase::VersionInfo::VersionScheme>(m, "VersionScheme")
        ...
        .value("numbersandletters",
               MOBase::VersionInfo::SCHEME_NUMBERSANDLETTERS)
    ...
    //   IPluginRequirementWrapper, py::bases<>,
    //   boost::noncopyable>("IPluginRequirement");
    // {
    //   py::scope scope = iPluginRequirementClass;

    //   py::class_<IPluginRequirement::Problem>("Problem",
    //     py::init<QString, QString>((py::arg("short_description"),
    //     py::arg("long_description") = ""))) .def("shortDescription",
    //     &IPluginRequirement::Problem::shortDescription)
    //     .def("longDescription",
    //     &IPluginRequirement::Problem::longDescription);

    //   iPluginRequirementClass
    //     .def("check", py::pure_virtual(&IPluginRequirement::check),
    //     py::arg("organizer"))
    //     ;
    // }
    ...
    //   // basic
    //   .def("basic", &PluginRequirementFactory::basic, (py::arg("checker"),
    //   "description")) .staticmethod("basic");
    ...
    //     py::return_value_policy<py::reference_existing_object>(),
    //     py::arg("name")) .def("modDataChanged", &IOrganizer::modDataChanged,
    //     py::arg("mod")) .def("isPluginEnabled", +[](IOrganizer* o, IPlugin*
    //     plugin) { return o->isPluginEnabled(plugin); }, py::arg("plugin"))
    ...
    //     std::function<bool(QString const&)> f) { return o->findFiles(p, f);
    //     },
    //       (py::arg("path"), "filter"))

    //     // In C++, it is possible to create a QStringList implicitly from a
    //     single QString. This is not possible with the current
    //     // converters in python (and I do not think it is a good idea to have
    //     it everywhere), but here it is nice to be able to
    ...
    //     .def("findFiles", +[](const IOrganizer* o, QString const& p, const
    //     QStringList& gf) { return o->findFiles(p, gf); },
    //       (py::arg("path"), "patterns"))
    //     .def("findFiles", +[](const IOrganizer* o, QString const& p, const
    //     QString& f) { return o->findFiles(p, QStringList{ f }); },
    //       (py::arg("path"), "pattern"))
    ...
    //     // Custom implementation for startApplication and waitForApplication
    //     because 1) HANDLE (= void*) is not properly
    //     // converted from/to python, and 2) we need to convert the by-ptr
    //     argument to a return-tuple for waitForApplication:
    //     .def("startApplication",
    //       +[](IOrganizer* o, const QString& executable, const QStringList&
    //       args, const QString& cwd, const QString& profile,
    //         const QString& forcedCustomOverwrite, bool ignoreCustomOverwrite)
    //         {
    //           return (std::uintptr_t) o->startApplication(executable, args,
    //           cwd, profile, forcedCustomOverwrite, ignoreCustomOverwrite);
    ...
    //     py::arg("callback")) .def("onUserInterfaceInitialized",
    ...
    //     py::arg("callback")) .def("onPluginEnabled", +[](IOrganizer* o,
    ...
    //     .def("onPluginDisabled", +[](IOrganizer* o, std::function<void(const
    //     IPlugin*)> const& func) {
    ...
    //     }, py::return_value_policy<py::reference_existing_object>(),
    //     py::arg("name")) .def("removeMod", +[](IOrganizer* o, IModInterface
    //     *mod) {
    ...
    //     (py::arg("game_name"), "mod_id", "user_data")) .def("requestFiles",
    //     &IModRepositoryBridge::requestFiles, (py::arg("game_name"), "mod_id",
    //     "user_data")) .def("requestFileInfo",
    ...
    //     "mod_id", "file_id", "user_data")) .def("requestToggleEndorsement",
    ...
    //     .def(py::init<py::optional<QString, int, int>>((py::arg("game_name"),
    //     "mod_id", "file_id"))) .def("__str__",
    //     &ModRepositoryFileInfo::toString) .def("createFromJson",
    //     &ModRepositoryFileInfo::createFromJson,
    ...
    //     .def_readwrite("newestVersion",
    //     &ModRepositoryFileInfo::newestVersion) .def_readwrite("categoryID",
    //     &ModRepositoryFileInfo::categoryID) .def_readwrite("modName",
    //     &ModRepositoryFileInfo::modName) .def_readwrite("gameName",
    //     &ModRepositoryFileInfo::gameName) .def_readwrite("modID",
    //     &ModRepositoryFileInfo::modID) .def_readwrite("fileID",
    //     &ModRepositoryFileInfo::fileID) .def_readwrite("fileSize",
    //     &ModRepositoryFileInfo::fileSize) .def_readwrite("fileName",
    //     &ModRepositoryFileInfo::fileName) .def_readwrite("fileCategory",
    ...
    //   // accept both QString and GuessedValue<QString> since the conversion
    //   is not automatic in Python, and
    ...
    //     py::arg("version")) .def("setIsEndorsed",
    //     &IModInterface::setIsEndorsed, py::arg("endorsed"))
    ...
    //     &IModInterface::addCategory, py::arg("name")) .def("removeCategory",
    //     &IModInterface::removeCategory, py::arg("name")) .def("setGameName",
    //     &IModInterface::setGameName, py::arg("name")) .def("setUrl",
    //     &IModInterface::setUrl, py::arg("url")) .def("pluginSetting",
    ...
    //          QString&)>(&GuessedValue<QString>::update), py::return_self<>(),
    //          py::arg("value"))
    //     .def("update",
    //          static_cast<GuessedValue<QString>&
    ...
    //     .def("reset", +[](GuessedValue<QString>* gv, const QString& value,
    //     EGuessQuality eq) {
    ...
    //     +[](GuessedValue<QString>* gv, std::function<std::variant<QString,
    //     bool>(QString const&)> fn) {
    ...
    //     // This makes a copy in python but it more practical than exposing an
    //     iterator: .def("variants", &GuessedValue<QString>::variants,
    //     py::return_value_policy<py::copy_const_reference>()) .def("__str__",
    //     &MOBase::GuessedValue<QString>::operator const QString&,
    ...
    //     &MOBase::IPluginList::loadOrder, py::arg("name")) .def("isMaster",
    //     &MOBase::IPluginList::isMaster, py::arg("name")) .def("masters",
    //     &MOBase::IPluginList::masters, py::arg("name")) .def("origin",
    //     &MOBase::IPluginList::origin, py::arg("name")) .def("onRefreshed",
    //     &MOBase::IPluginList::onRefreshed, py::arg("callback"))
    ...
    //     .def("setState", &MOBase::IPluginList::setState, (py::arg("name"),
    //     "state")) .def("setLoadOrder", &MOBase::IPluginList::setLoadOrder,
    //     py::arg("loadorder"))
    //     ;
    ...
    //       bool)>(&MOBase::IModList::setActive), (py::arg("names"), "active"))
    //     .def("setActive",
    //       static_cast<bool(IModList::*)(QString const&,
    //       bool)>(&MOBase::IModList::setActive), (py::arg("name"), "active"))
    //     .def("priority", &MOBase::IModList::priority, py::arg("name"))
    //     .def("setPriority", &MOBase::IModList::setPriority, (py::arg("name"),
    //     "priority"))
    ...
    //         "onModStateChanged(Callable[[str, IModList.ModStates], None]) is
    //         deprecated, " "use onModStateChanged(Callable[[Dict[str,
    ...
    // // Note: localizedName, master, requirements and enabledByDefault have to
    // go in all the plugin wrappers declaration,
    // // since the default functions are specific to each wrapper, otherwise in
    // turns into an
    ...
    //   py::pure_virtual(&MOBase::IPlugin::settings)) .def("enabledByDefault",
    ...
    //     &MOBase::IPlugin::master, &IPluginDiagnoseWrapper::master_Default)
    //     .def("requirements", &MOBase::IPlugin::requirements,
    ...
    //     .def("__init__", py::make_constructor(+[](QString src, QString dst,
    //     bool dir, bool crt) -> Mapping* {
    ...
    //     &MOBase::IPlugin::master, &IPluginFileMapperWrapper::master_Default)
    //     .def("requirements", &MOBase::IPlugin::requirements,
    ...
    // // This doesn't actually do the conversion, but might be convenient for
    // accessing the names for enum bits
    ...
    //     .def("detectGame",
    //     py::pure_virtual(&MOBase::IPluginGame::detectGame)) .def("gameName",
    ...
    //     py::pure_virtual(&MOBase::IPluginGame::listSaves), py::arg("folder"))
    //     .def("isInstalled",
    //     py::pure_virtual(&MOBase::IPluginGame::isInstalled)) .def("gameIcon",
    ...
    //     .def("setGamePath",
    //     py::pure_virtual(&MOBase::IPluginGame::setGamePath), py::arg("path"))
    ...
    //     .def("DLCPlugins",
    //     py::pure_virtual(&MOBase::IPluginGame::DLCPlugins)) .def("CCPlugins",
    ...
    //         py::converter::registry::query(py::type_id<T>()); py::object key
    //         =
    ...
    // py::class_<IPluginInstallerSimpleWrapper, py::bases<IPluginInstaller>,
    ...
    //   &MOBase::IPlugin::localizedName,
    //   &IPluginInstallerSimpleWrapper::localizedName_Default) .def("master",
    //   &MOBase::IPlugin::master,
    //   &IPluginInstallerSimpleWrapper::master_Default) .def("requirements",
    ...
    //   // Note: Keeping the variant here even if we always return a tuple to
    //   be consistent with the wrapper and
    //   // have proper stubs generation.
    //   .def("install", +[](IPluginInstallerSimple* p, GuessedValue<QString>&
    //   modName, std::shared_ptr<IFileTree>& tree, QString& version, int&
    //   nexusID)
    ...
    // py::class_<IPluginInstallerCustomWrapper, py::bases<IPluginInstaller>,
    ...
    //   py::arg("new_mod"))) .def("localizedName",
    //   &MOBase::IPlugin::localizedName,
    //   &IPluginInstallerCustomWrapper::localizedName_Default) .def("master",
    //   &MOBase::IPlugin::master,
    //   &IPluginInstallerCustomWrapper::master_Default) .def("requirements",
    //   &MOBase::IPlugin::requirements,
    ...
    //   &IPluginInstallerCustom::isArchiveSupported, py::arg("archive_name"))
    //   .def("supportedExtensions",
    //   &IPluginInstallerCustom::supportedExtensions) .def("install",
    //   &IPluginInstallerCustom::install, (py::arg("mod_name"), "game_name",
    //   "archive_name", "version", "nexus_id")) .def("_parentWidget",
    //   &IPluginInstallerSimpleWrapper::parentWidget,
    //   py::return_value_policy<py::return_by_value>()) .def("_manager",
    ...
    //   .def("requirements", &MOBase::IPlugin::requirements,
    //   &IPluginModPageWrapper::requirements_Default) .def("enabledByDefault",
    //   &MOBase::IPlugin::enabledByDefault,
    ...
    //   .def("requirements", &MOBase::IPlugin::requirements,
    //   &IPluginPreviewWrapper::requirements_Default) .def("enabledByDefault",
    //   &MOBase::IPlugin::enabledByDefault,
    ...
    m.def("getProductVersion", &MOBase::getProductVersion,
          py::arg("executable"));
    m.def("getIconForExecutable", &MOBase::iconForExecutable,
          py::arg("executable"));
    ...
    // mobase.MoVariant)). The real MoVariant is defined in the generated stubs,
    // since it's only relevant when doing type-checking, but this needs to be
    // defined, otherwise MoVariant is not found when actually running plugins
    // through MO2, making them crash.
...
template <class T>
void PythonRunner::appendIfInstance(py::object const& obj,
    ...
    try {
        ...
        if (moduleDict.contains("createPlugin")) {
            ...
            if (!PySequence_Check(pyPlugins.ptr())) {
                MOBase::log::error(
                    "Plugin {}: createPlugins must return a list.", identifier);
            }
            else {
        ...
        for (py::object pluginObj : plugins) {
            ...
            if (interfaceList.isEmpty()) {
                MOBase::log::error(
                    "Plugin {}: no plugin interface implemented.", identifier);
            }
